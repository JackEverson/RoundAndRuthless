...
bool SimpleSoundManager::LoadSound(const std::string& name, const std::string& filepath) {
    ...
    if (alGetError() != AL_NO_ERROR) {
        std::cerr << "Failed to create OpenAL buffer/source for " << name << std::endl;
        return false;
    }
...
bool SimpleSoundManager::LoadWAV(const std::string& filepath, ALuint buffer) {
    ...
    if (wav.channels == 1) {
        ...
        std::cerr << "Unsupported channel count in WAV: " << wav.channels << std::endl;
        drwav_uninit(&wav);
        return false;
    }
    ...
    alBufferData(buffer, format, samples.data(), 
                 samples.size() * sizeof(int16_t), wav.sampleRate);
    ...
    std::cout << "WAV loaded: " << wav.channels << " channels, " 
              << wav.sampleRate << " Hz, " << wav.totalPCMFrameCount << " frames" << std::endl;
...
bool SimpleSoundManager::LoadOGG(const std::string& filepath, ALuint buffer) {
    ...
    if (!vorbis) {
        std::cerr << "Failed to load OGG file: " << filepath << " (error: " << error << ")" << std::endl;
        return false;
    }
    ...
    stb_vorbis_get_samples_short_interleaved(vorbis, info.channels, samples.data(), sampleCount);
    ...
    if (info.channels == 1) {
        ...
        std::cerr << "Unsupported channel count in OGG: " << info.channels << std::endl;
    ...
    // Upload to OpenAL
    alBufferData(buffer, format, samples.data(), 
                 samples.size() * sizeof(int16_t), info.sample_rate);
...
void SimpleSoundManager::PlaySound(const std::string& name, bool loop) {
    ...
    if (alGetError() != AL_NO_ERROR) {
        ...
        std::cout << "Playing " << (loop ? "looping " : "") << "sound: " << name << std::endl;
    }
}
...
// Helper function to update sound volume (considers master volume and music volume)
...
void SimpleSoundManager::PlayBackgroundMusic(const std::string& name, float volume) {
    // Stop current background music if any
    if (!currentBackgroundMusic.empty() && IsSoundPlaying(currentBackgroundMusic)) {
...
void SimpleSoundManager::StopBackgroundMusic() {
    if (!currentBackgroundMusic.empty()) {
        StopSound(currentBackgroundMusic);
        std::cout << "Stopped background music: " << currentBackgroundMusic << std::endl;
